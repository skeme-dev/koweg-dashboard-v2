<script lang="ts">
	...
	let publishNow: boolean = $state.raw(data.currentPost.publisDate ? false : true);
	let publishDate = $state.raw(data.currentPost.publishDate);
...
<div class="space-y-6">
	...
	<div class="flex flex-col w-full gap-10">
		...
		<Button
			class="ml-auto w-fit"
			disabled={!publishNow && !publishDate}
			onclick={() => {
				const updateData = {
					isPublished: publishNow,
					publishDate: publishNow ? '' : publishDate.toDate()
				};

				if (Object.keys(updateData).length == 0) {
					console.log('Empty updateData');
					return toast.error('Fehlerbeim Speichern der Veröffentlichungen');
				}
				...
				updatePost(
					...
					updateData,
					() => {
						toast.success('Veröffentlichungen erfolgreich gespeichert.');
					},
					(error) => {
						console.error(error);
						return toast.error('Fehler beim Speichern der Veröffentlichungen.');
					}
				);
			}}>Speichern</Button
		>
